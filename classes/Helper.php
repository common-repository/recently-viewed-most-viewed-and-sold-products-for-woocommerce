...
class Helper extends Base {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "rvpfw_{$_args['post_type']}", 'rvpfw' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "rvpfw_{$_args['post_type']}", $posts, 'rvpfw', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'rvpfw' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'rvpfw_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /recently-viewed-products-for-woocommerce directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with recently-viewed-products-for-woocommerce_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'rvpfw_template_overwrite_dir', get_stylesheet_directory() . '/recently-viewed-products-for-woocommerce/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( RVPFW ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'recently-viewed-products-for-woocommerce' );
		}
	}
}